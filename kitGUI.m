...
function hs = createControls()
  ...
  % Buttons for selecting directory the movies are contained in
  hs.selectDirectoryFolders = button(hs.fig,'Folders',[w-32 0.865*figh 17.5 2],@selectDirectoryFoldersCB);
  hs.selectDirectory = button(hs.fig,'Files',[w-15 0.865*figh 17.5 2],@selectDirectoryCB);
  ...
  %Region of Interest Buttons
  hs.addROI = button(hs.fig,'Add ROI',[x 1 13 2],@addROICBFolders);
...
function selectDirectoryFoldersCB(hObj,event)
  if ~isempty(get(handles.ROIs,'String'))
    r = questdlg('Selecting movie directory will clear existing ROIs. Select?','Warning','Yes','No','No');
    if strcmp(r,'No')
      return
...
%also new function here
function addROICBFolders(hObj,event)
  
  movieDir = handles.movieDirectory.String;
  
  v = handles.movies.Value;
  movieFolder = handles.movies.String{v};
  
  movieDir = strcat(movieDir,'/',movieFolder);
  
  movieFilesDir = dir(movieDir)
  movieFiles = {};
  for i = 1:length(movieFilesDir)
  	  if ~movieFilesDir(i).isdir
  	  
  	  movieFiles = [movieFiles, movieFilesDir(i).name];
  ...
  for i=1:length(movieFiles)
    [crop,cropSize] = kitCropMovie(fullfile(movieDir,movieFiles{i}));
    if ~isempty(crop)
      for j=1:size(crop,1)
        r = length(jobset.ROI) + 1;
        jobset.ROI(r).movie = handles.movies.String{v(i)};
...
function populateMovieBoxFolders()
  movieDir = handles.movieDirectory.String;
  if isempty(movieDir)
    handles.movies.String = [];
  else
    % Find movie files.
    movieFiles = kitFindFolders(movieDir, kitSupportedFormats(),0,1);
    % Strip search directory from filenames.
    for i=1:length(movieFiles)
      movieFiles{i} = strrep(movieFiles{i},[movieDir filesep],'');
    end
